<script lang="ts">
    ...
    async function updateFileStatus(id: number, currStatus: string) {
        ...
        await callAPI({
            host: $hostname, endpoint: `/api/file/status/${ id }?status=${ newStatus }`, method: 'PUT',
            callback: async (res) => {
...
<div class="p-2 relative">
    <a class="" href="/files/{id}">
        <img alt="img" class="rounded-xl w-52 h-52 object-cover shadow-xl" src="{getImgUrl(filename)}">
        <!-- <img alt="img" class="rounded-xl shadow-xl" src="{getImgUrl(filename)}"> -->
    </a>

    {#if status === 'inbox'}
        <div class="bg-slate-800/50 w-52 h-6 absolute top-2 rounded-t-xl flex justify-end">
            <svg width="20px" height="20px" viewBox="0 0 128 128" class="{color} mt-1 mr-1.5"
                 on:click={updateFileStatus(id, status)}>
